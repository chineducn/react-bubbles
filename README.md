...
- [x] Explain what a token is used for.
    ...
    A token is a string that the server generates as an identity for a particular login instance.
...
- [x] Describe how web servers work.
    ...
    A web server is a computer that holds data/resources, has a hostname and url. With the hostname and url, requests can be made to the server. If the request is approved, the data is sent to the requester.
...
### API Documentation
  ...
  * **[POST]** * to `/api/login`: returns a token to be added to the header of all other requests. Pass in the following credentials as the `body` of the request: `{ username: 'lambda', password: '1234' }`
